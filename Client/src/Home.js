        ...
        .then(
          (result) => {
            ...
            if(window.localStorage.getItem('toggle')=="true"){
              ...
              var final_array=[]
              this.check_validity(result_array,final_array)
              var component_array = final_array.map(function(item,index){
              ...
              var finalarr=[]
              this.check_validity_nwtimes(result_array,finalarr)
              var component_array = finalarr.map(function(item,index){
                var date_format = item.published_date.split('T')[0]
                var multimedia = item.multimedia
                var img=""
                if(multimedia!=null){
                  var len = multimedia.length
                }
                else{
                  var len=0
                }
